...
def upload_upf_family(folder, group_label, group_description, stop_if_existing=True, backend=None):
    ...
    automatic_user = orm.User.get_collection(backend).get_default()
    group, group_created = orm.UpfFamily.get_collection(backend).get_or_create(label=group_label, user=automatic_user)
